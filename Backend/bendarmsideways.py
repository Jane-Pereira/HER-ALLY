...
# Function to detect bent arm sideways exercise and provide feedback
def detect_bent_arm_sideways(landmarks):
    ...
    correct_angle_threshold = 180  # degrees

    left_correct = np.abs(left_arm_angle - correct_angle_threshold) < 10
    right_correct = np.abs(right_arm_angle - correct_angle_threshold) < 10
...
try:
    font_path = "/path/to/Times_New_Roman.ttf"  
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        left_correct, right_correct = detect_bent_arm_sideways(results.pose_landmarks.landmark)

        # Check if both arms are down
        left_arm_down = results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST].y > results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_HIP].y
        right_arm_down = results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST].y > results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HIP].y
        ...
        if left_correct and right_correct:
            feedback = 'Bent Arm Sideways Correct'
            color = (0, 255, 0)
            incorrect_posture_start_time = None  # Reset the incorrect posture timer
        else:
            feedback = 'Bent Arm Sideways Incorrect'
            color = (0, 0, 255)
    ...
    text = "Exercise 3: Bent Arm Sideways"
    ...
    cv2.imshow('Bent Arm Sideways Detection', annotated_frame)
